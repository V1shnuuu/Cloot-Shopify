...
## AI Architecture Source of Truth

- Always read and follow the Mermaid architecture documentation in `docs/` before generating or changing code.
- Treat `docs/architecture.md`, `docs/ecommerce-system.mmd`, `docs/checkout-flow.mmd`, `docs/admin-flow.mmd`, `docs/inventory-flow.mmd`, `docs/recommendation-flow.mmd`, `docs/returns-flow.mmd`, `docs/frontend-flow.mmd`, `docs/backend-flow.mmd`, and `docs/database-design.mmd` as source-of-truth development context.
- Follow the Mermaid diagrams strictly. If implementation needs to diverge, update the relevant diagram in the same change.
- Maintain the existing Shopify modular structure: `sections`, `snippets`, `templates`, `blocks`, `assets`, `config`, `layout`, and `locales`.
- Reuse existing components, snippets, section patterns, schema conventions, CSS utilities, and naming conventions whenever possible.
- Keep business logic isolated: theme Liquid should render storefront behavior, Shopify platform APIs should own commerce data, and custom backend behavior should live in apps, app proxies, webhooks, message brokers, or external services.
- Preserve Shopify cart flow, checkout redirects, product collection behavior, metafields, app integrations, analytics hooks, and theme editor configurability.
- Preserve existing architecture consistency across storefront, checkout, admin, inventory, recommendations, returns, analytics, notifications, and integrations.
- Update the relevant Mermaid diagram whenever architecture, data relationships, API usage, app integration boundaries, cart flow, checkout flow, admin flow, inventory flow, recommendation flow, returns flow, or frontend flow changes.
- Keep generated code production-grade, modular, and aligned with the architecture diagrams before adding new abstractions.
